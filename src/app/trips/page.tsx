"use client";
import {Shell} from "@/components/shell";import {trips as defaultTrips,yen} from "@/lib/data";import {CalendarDays,Edit3,MapPin,Plus,Save} from "lucide-react";import {useEffect,useMemo,useState} from "react";import {createClient} from "@/lib/supabase/client";
...
export default function Trips(){
  ...
  async function saveTrip(){
    ...
    const nextItems=key==="new"||!key
      ? [...items,nextDraft]
      : items.map((item)=>(item.id??item.name)===key?nextDraft:item);
    setItems(nextItems);
